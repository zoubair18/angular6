...
export class UserService {
  private baseUrl = 'http://213.32.91.120:8080/robbin/api';
  ...
  getUsers() {
    return this._http.get(this.baseUrl + '/user/getAll', this.options).map((response: Response) => response.json())
  ...
  createUser(user: User) {
    return this._http.post(this.baseUrl + '/user/save', JSON.stringify(user), this.options).map((response: Response) => response.json())
      .catch(this.errorHandler);
  }
